...
export function ensureGameMeta(gameMeta, gameId, { initialSent } = {}) {
  ...
  if (!(meta.disconnected instanceof Set)) {
    meta.disconnected = new Set();
  }
  if (!meta.lastSeen || typeof meta.lastSeen !== 'object') {
    meta.lastSeen = Object.create(null);
  }
  if (typeof meta.createdAt !== 'number' || !Number.isFinite(meta.createdAt)) {
    meta.createdAt = Date.now();
  }

  if (typeof meta.initialSent !== 'boolean') {
    meta.initialSent = false;
  }
  if (typeof initialSent === 'boolean') {
    meta.initialSent = meta.initialSent || initialSent;
  }
...
export function ensureGameResult(meta, patch = {}) {
  ...
  const normalized = (r) => {
    ...
    if (typeof e.winner === 'undefined') {
      e.winner = 'winner' in p ? p.winner : null;
    }
