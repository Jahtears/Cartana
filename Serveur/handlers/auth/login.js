...
function buildMaxTryError(retryAfterMs) {
  ...
  return {
    code: ERROR.AUTH_RATE_LIMITED,
    params: { retry_after_s: retryAfterS },
...
export async function handleLogin(ctx, ws, req, data) {
  ...
  if (!username || !pin) {
    resError(sendRes, ws, req, ERROR.AUTH_MISSING_CREDENTIALS);
    return true;
  }
  ...
  try {
    ...
    if (state.getWS(username)) {
      sendRes(ws, req, false, {
        code: ERROR.AUTH_ALREADY_CONNECTED,
      });
      return true;
    }
    ...
    resError(sendRes, ws, req, ERROR.INTERNAL_ERROR);
    return true;
  }
}
