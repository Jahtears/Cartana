...
export function handleSpectateGame(ctx, ws, req, data, actor) {
  ...
  const { sendRes, state, refreshLobby, sendEvtSocket, attachSpectator } = ctx;
  const { emitStartGameToUser, emitFullState } = sessionUsecases;
  ...
  if (rejectIfBusyOrRes(ctx, ws, req, actor, POPUP_MESSAGE.TECH_BAD_STATE)) {
    return true;
  }
  ...
  if (alreadySpectatingThisGame && meta.initialSent) {
