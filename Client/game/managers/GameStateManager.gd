...
func _on_evt_start_game(game_id: String, players: Array, spectator: bool) -> void:
  # 1. Mettre à jour GameSession (source de vérité)
  GameSession.start_game(game_id, players, bool(spectator))

  # 2. Reset des flags de session dans le contexte
  ...
  # 5. Sync des flags sur game_node
  ...
  # 6. Demander le snapshot via ClientAPI
  if GameSession.current_game_id != "":
    if GameSession.is_spectator:
      ClientAPI.spectate_game(GameSession.current_game_id)
    else:
      ClientAPI.join_game(GameSession.current_game_id)
  ...
  if _board_sync_service != null:
    for slot_id in slots_dict.keys():
      ...
      if context.game_node != null and "_deck_count_state" in context.game_node:
        DeckCountUtil.update_from_slot(context.game_node._deck_count_state, slot_id, count_for_slot)
...
func _on_login_ok(_username: String, _status: Dictionary) -> void:
  if GameSession.current_game_id != "":
    _request_game_sync()
...
func _request_game_sync() -> void:
  var game_id: String = String(GameSession.current_game_id)
  if game_id == "":
    return
  if GameSession.is_spectator:
...
func on_server_closed(_server_reason: String, _close_code: int) -> void:
  context.network_disconnected = false
...
func _resolve_rematch_target_username() -> String:
  ...
  for player in GameSession.players_in_game:
    if player is String:
