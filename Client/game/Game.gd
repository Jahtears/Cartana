...
func _leave_current_and_go_lobby() -> void:
  ...
  if gid != "":
    if GameSession.is_game_ended() or GameSession.is_spectator:
      # fire-and-forget : on ne bloque pas le retour au lobby
      ClientAPI.ack_game_end(gid)
    else:
...
func _exit_tree() -> void:
  # Notifier le serveur si une partie est encore active (fire-and-forget)
  if GameSession.current_game_id != "":
    ClientAPI.ack_game_end(GameSession.current_game_id)

  if game_state_manager != null:
    # Déconnecter les signaux ClientAPI connectés dans _connect_network_signals
    var signal_methods: Array = [
      [ClientAPI.evt_start_game,             "_on_evt_start_game"],
      [ClientAPI.evt_state_snapshot,         "_on_evt_state_snapshot"],
      [ClientAPI.evt_slot_state,             "_on_evt_slot_state"],
      ...
      [ClientAPI.evt_rematch_declined,       "_on_evt_rematch_declined"],
      [ClientAPI.join_game_ok,               "_on_join_game_ok"],
      [ClientAPI.move_ok,                    "_on_move_ok"],
      [ClientAPI.move_failed,                "_on_move_failed"],
      [ClientAPI.invite_sent,                "_on_invite_sent"],
      [ClientAPI.invite_send_failed,         "_on_invite_send_failed"],
      [ClientAPI.login_ok,                   "_on_login_ok"],
      [ClientAPI.connection_lost,            "on_connection_lost"],
      [ClientAPI.connection_restored,        "on_connection_restored"],
      [ClientAPI.reconnect_failed,           "on_reconnect_failed"],
      [ClientAPI.server_closed,              "on_server_closed"],
    ]
    for pair in signal_methods:
      var sig: Signal = pair[0]
      var method: String = pair[1]
      if game_state_manager.has_method(method):
        var c := Callable(game_state_manager, method)
...
func _notification(what: int) -> void:
  if what == NOTIFICATION_WM_CLOSE_REQUEST and GameSession.current_game_id != "":
    # fire-and-forget à la fermeture de la fenêtre
    ClientAPI.ack_game_end(GameSession.current_game_id)
